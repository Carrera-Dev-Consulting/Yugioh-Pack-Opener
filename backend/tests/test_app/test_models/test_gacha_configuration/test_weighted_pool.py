...
import pytest
from app.models.gacha_configuration import Weight, WeightedPool


def test_when_weighted_pool_total_does_not_work__raise_value_error():
    pool = WeightedPool(quantity=1, weights=[])
...
def test_when_random_selects_less_then_light_weight__picks_light_weight_item(random):
    low_weight = Weight(cards=[], weight=0.1)
    high_weight = Weight(cards=[], weight=0.9)
    sut = WeightedPool(quantity=1, weights=[low_weight, high_weight])
...
def test_when_random_selects_gets_one_greater_then_first_percentage_but_less_than_second__returns_second_weight(
    ...
    low_weight = Weight(cards=[], weight=0.1)
    other_low_weight = Weight(cards=[], weight=0.2)
    high_weight = Weight(cards=[], weight=0.7)
    sut = WeightedPool(quantity=1, weights=[low_weight, high_weight, other_low_weight])
    random.return_value = 0.1
    picked = sut.pick_weight(random_float=random)
...
def test_when_weights_are_greater_than_one__still_selects_weights_properly(random):
    low_weight = Weight(cards=[], weight=0.16)
    other_low_weight = Weight(cards=[], weight=0.14285)
    other_lowish_weight = Weight(cards=[], weight=0.2)
    high_weight = Weight(cards=[], weight=1)
